package com.charter.camunda.worker.validation.handler;

import com.charter.camunda.worker.validation.exception.TechnicalException;
...
public class ExternalTaskErrorHandler {
    ...
    private static final long BASE_RETRY_TIMEOUT = 5000; // 5 seconds
    ...
    public void handleError(ExternalTask task, ExternalTaskService service, Exception exception) {
        ...
        if (exception instanceof ValidationException) {
            handleValidationError(task, service, (ValidationException) exception, correlationId);
    ...
    private void handleValidationError(ExternalTask task, ExternalTaskService service, 
        ...
        if (ex.isRetryable()) {
            ...
            log.warn("Validation error (retryable) | taskId={} | correlationId={} | retriesLeft={} | retryIn={}ms", 
                    task.getId(), correlationId, retries, retryTimeout);

            service.handleFailure(task, ex.getMessage(), ex.toString(), retries - 1, retryTimeout);
        } else {
            log.error("Validation error (non-retryable) | taskId={} | correlationId={} | errorCode={}", 

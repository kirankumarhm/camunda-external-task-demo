package com.charter.camunda.worker.fraud.handler;

import com.charter.camunda.worker.fraud.exception.FraudException;
import com.charter.camunda.worker.fraud.exception.TechnicalException;
...
public class ExternalTaskErrorHandler {
    ...
    public void handleError(ExternalTask task, ExternalTaskService service, Exception exception) {
        ...
        if (exception instanceof FraudException) {
            handleFraudError(task, service, (FraudException) exception, correlationId);
    ...
    private void handleFraudError(ExternalTask task, ExternalTaskService service, 
                                  FraudException ex, String correlationId) {
        ...
        if (ex.isRetryable()) {
            ...
            log.warn("Fraud error (retryable) | taskId={} | correlationId={} | retriesLeft={} | retryIn={}ms", 
                    task.getId(), correlationId, retries, retryTimeout);

            service.handleFailure(task, ex.getMessage(), ex.toString(), retries - 1, retryTimeout);
        } else {
            log.error("Fraud error (non-retryable) | taskId={} | correlationId={} | errorCode={}", 
